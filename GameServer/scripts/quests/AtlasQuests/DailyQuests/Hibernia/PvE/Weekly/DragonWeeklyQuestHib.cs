	...
	public class DragonWeeklyQuestHib : Quests.WeeklyQuest
	{
				...
				if (quest != null)
				{
					switch (quest.Step)
					{
						case 1:
							...
							break;
						case 2:
							Dean.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.DidYouToken", player.Name, "[slay the dragon] and return for your reward"));
							break;
					}
				}
				else
				{
					Dean.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.DragonIntro", player.Name, "Dean", DRAGON_NAME, "Sheeroe Hills", "Hibernia"));
				}
			}
			...
			else if (e == GameLivingEvent.WhisperReceive)
			{
				...
				if (quest == null)
				{
					switch (wArgs.Text)
					{
						...
						case "드래곤 처치":
							player.Out.SendQuestSubscribeCommand(Dean, QuestMgr.GetIDForQuestType(typeof(DragonWeeklyQuestHib)), DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.HelpNpcWithQuest", "Dean", questTitle));
							break;
					}
				}
				else
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				Dean.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.DragonReminder", "Sheeroe Hills"));
		...
		public override string Description
		{
			get
			{
				...
				{
					case 1:
						return DOL.Language.LanguageMgr.GetTranslation(m_questPlayer.Client.Account.Language, "Quest.Common.DragonDescription", "Sheeroe Hills", DRAGON_NAME, "Hibernia", DragonKilled, MAX_KILLED);
					case 2:
						return DOL.Language.LanguageMgr.GetTranslation(m_questPlayer.Client.Account.Language, "Quest.Common.ReturnToNpc", "Dean");
				}
