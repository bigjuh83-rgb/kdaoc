		...
		private static void TalkToRey(DOLEvent e, object sender, EventArgs args)
		{
				...
				if (quest != null)
				{
					switch (quest.Step)
					{
						case 1:
							...
							break;
						case 2:
							ReyHib.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.HitQuotaPrompt", player.Name));
							break;
					}
				}
				else
				{
					ReyHib.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.ReyDailyBonesIntro", player.Name));
				}
			}
			...
			else if (e == GameLivingEvent.WhisperReceive)
			{
				...
				if (quest == null)
				{
					...
					{
						case "help me out":
							player.Out.SendQuestSubscribeCommand(ReyHib, QuestMgr.GetIDForQuestType(typeof(PlayerKillQuestHib)), DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.SubscribePrompt", questTitle));
							break;
					}
				}
				else
