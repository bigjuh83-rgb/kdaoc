	...
	public class EveryLittleBitHelpsQuestHib : Quests.DailyQuest
	{
		...
		protected static void TalkToRey(DOLEvent e, object sender, EventArgs args)
		{
			...
			if (e == GameObjectEvent.Interact)
			{
				if (quest != null)
				{
					...
					{
						case 1:
							ReyHib.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.KillEnemiesInFrontiers", "Midgard", "Albion"));
							break;
						case 2:
							ReyHib.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.DidYouToken", player.Name, "[kill enemies] for your reward"));
							break;
					}
				}
				else
				{
					ReyHib.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.ReyExoticBonesFlavor", player.Name, "Hibernia"));
				}
			}
			...
			else if (e == GameLivingEvent.WhisperReceive)
			{
				...
				if (quest == null)
				{
					switch (wArgs.Text)
					{
						...
						case "발가락뼈 가져오기":
						case "발가락뼈":
							player.Out.SendQuestSubscribeCommand(ReyHib, QuestMgr.GetIDForQuestType(typeof(EveryLittleBitHelpsQuestHib)), DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.SubscribePrompt", questTitle));
							break;
					}
				}
				else
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			else
			{
				...
				ReyHib.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.SuitablePlayersFrontiers"));
		...
		public override string Description
		{
			get
			{
				...
				{
					case 1:
						return DOL.Language.LanguageMgr.GetTranslation(m_questPlayer.Client.Account.Language, "Quest.Common.TwoRealmPlayersKilled", DOL.Language.LanguageMgr.GetTranslation(m_questPlayer.Client.Account.Language, "Quest.Common.SuitablePlayersFrontiers"), "Albion", _playersKilledAlb, MAX_KILLGOAL, "Midgard", _playersKilledMid, MAX_KILLGOAL);
					case 2:
						return DOL.Language.LanguageMgr.GetTranslation(m_questPlayer.Client.Account.Language, "Quest.Common.ReturnToNpcLocation", "Rey", "Druim Ligen");
				}
		...
		public override void Notify(DOLEvent e, object sender, EventArgs args)
		{
			...
			{
				_playersKilledAlb++;
				player.Out.SendMessage(DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.DailyRealmEnemyKilled", "Albion", _playersKilledAlb, MAX_KILLGOAL), eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
				player.Out.SendQuestUpdate(this);
			}
