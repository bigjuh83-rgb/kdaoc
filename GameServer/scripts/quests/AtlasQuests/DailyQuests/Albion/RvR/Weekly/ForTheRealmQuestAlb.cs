	...
	public class ForTheRealmQuestAlb : Quests.WeeklyQuest
	{
		...
		private static void TalkToRey(DOLEvent e, object sender, EventArgs args)
		{
			...
			if (e == GameObjectEvent.Interact)
			{
				if (quest != null)
				{
					...
					{
						case 1:
							ReyAlb.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.SlayEnemyForcesSpecific", "Midgard", "Hibernia"));
							break;
						case 2:
							ReyAlb.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.DidYouToken", player.Name, "[slay their forces] for your reward"));
							break;
					}
				}
				else
				{
					ReyAlb.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.ReyWeeklyBonesFlavor", player.Name));
				}
			}
			...
			else if (e == GameLivingEvent.WhisperReceive)
			{
				...
				if (quest == null)
				{
					switch (wArgs.Text)
					{
						...
						case "발가락뼈 한 무더기":
							player.Out.SendQuestSubscribeCommand(ReyAlb, QuestMgr.GetIDForQuestType(typeof(ForTheRealmQuestAlb)), DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.SubscribePrompt", questTitle));
							break;
					}
				}
				else
		...
		public override string Description
		{
			get
			{
				...
				{
					case 1:
						return DOL.Language.LanguageMgr.GetTranslation(m_questPlayer.Client.Account.Language, "Quest.Common.TwoRealmPlayersKilled", DOL.Language.LanguageMgr.GetTranslation(m_questPlayer.Client.Account.Language, "Quest.Common.SlayEnemyForces"), "Hibernia", _playersKilledHib, MAX_KILLGOAL, "Midgard", _playersKilledMid, MAX_KILLGOAL);
					case 2:
		...
		public override void Notify(DOLEvent e, object sender, EventArgs args)
		{
			...
			{
				_playersKilledMid++;
				player.Out.SendMessage(DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.WeeklyRealmEnemyKilled", "Midgard", _playersKilledMid, MAX_KILLGOAL), eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
			...
			{
				_playersKilledHib++;
				player.Out.SendMessage(DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.WeeklyRealmEnemyKilled", "Hibernia", _playersKilledHib, MAX_KILLGOAL), eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
				player.Out.SendQuestUpdate(this);
			}
