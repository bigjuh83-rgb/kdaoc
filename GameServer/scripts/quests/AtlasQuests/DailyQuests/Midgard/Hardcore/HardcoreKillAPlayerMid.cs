...
namespace DOL.GS.DailyQuest
{
	public class HardcoreKillAPlayerMid : Quests.DailyQuest
	{
		...
		private static void TalkToSucci(DOLEvent e, object sender, EventArgs args)
		{
			...
			if (e == GameObjectEvent.Interact)
			{
				if (oranges != null)
				{
					...
					{
						case 1:
							SucciMid.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Hardcore.HuntOrBeHunted"));
							break;
						case 2:
				...
				{
					SucciMid.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Hardcore.PredatorIntro"));
			...
			else if (e == GameLivingEvent.WhisperReceive)
			{
				...
				if (oranges == null)
				{
					...
					{
						case "predator":
							player.Out.SendQuestSubscribeCommand(SucciMid, QuestMgr.GetIDForQuestType(typeof(HardcoreKillAPlayerMid)), DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.SubscribePrompt", questTitle));
							break;
					}
				}
				else
		...
		private static void CheckPlayerAcceptQuest(GamePlayer player, byte response)
		{
			...
			if (response == 0x00)
			{
				player.Out.SendMessage(DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Hardcore.LookThemInEye"), eChatType.CT_Say, eChatLoc.CL_PopupWindow);
			}
			else
			{
				...
				SucciMid.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Hardcore.HuntOnlyOneEats"));
		...
		public override string Description
		{
			get
			{
				switch (Step)
				{
					...
					case 1:
						return DOL.Language.LanguageMgr.GetTranslation(m_questPlayer.Client.Account.Language, "Quest.Hardcore.KillPlayerDescription", PlayerKilled, MAX_KillGoal);
					case 2:
						return DOL.Language.LanguageMgr.GetTranslation(m_questPlayer.Client.Account.Language, "Quest.Hardcore.ReturnToSucciMid");
				}
		...
		public override void Notify(DOLEvent e, object sender, EventArgs args)
		{
			...
			PlayerKilled = 1;
			player.Out.SendMessage(DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Hardcore.EnemyKilled", PlayerKilled, MAX_KillGoal), eChatType.CT_ScreenCenter, eChatLoc.CL_SystemWindow);
		...
		public override void FinishQuest()
		{
			...
			if (reward > 0)
			{
				m_questPlayer.Out.SendMessage(DOL.Language.LanguageMgr.GetTranslation(m_questPlayer.Client.Account.Language, "Quest.Common.RealmPointReward", reward, "Hardcore"), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
