		...
		private static void TalkToRey(DOLEvent e, object sender, EventArgs args)
		{
				...
				if (quest != null)
				{
					switch (quest.Step)
					{
						case 1:
							...
							break;
						case 2:
							ReyMid.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.HitQuotaPrompt", player.Name));
							break;
					}
				}
				else
				{
					ReyMid.SayTo(player, DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.ReyDailyBonesIntro", player.Name));
				}
			}
			...
			else if (e == GameLivingEvent.WhisperReceive)
			{
				...
				if (quest == null)
				{
					...
					{
						case "help me out":
							player.Out.SendQuestSubscribeCommand(ReyMid, QuestMgr.GetIDForQuestType(typeof(PlayerKillQuestMid)), DOL.Language.LanguageMgr.GetTranslation(player.Client.Account.Language, "Quest.Common.SubscribePrompt", questTitle));
							break;
					}
				}
				else
