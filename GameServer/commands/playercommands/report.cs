...
namespace DOL.GS.Commands
{
	...
	public class ReportCommandHandler : AbstractCommandHandler, ICommandHandler
	{
		...
		public void OnCommand(GameClient client, string[] args)
		{
			...
			{
				DisplayMessage(client, LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Report.Disabled"));
				return;
			}
			...
			{
				client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Report.Muted"), eChatType.CT_Staff, eChatLoc.CL_SystemWindow);
				return;
			}
			...
			if (ServerProperties.Properties.MAX_BUGREPORT_QUEUE > 0)
			{
				...
				if (found)
				{
					client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Report.QueueFull"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
					return;
				}
			...
			client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Report.Submitted"), eChatType.CT_System, eChatLoc.CL_SystemWindow);

			if (ServerProperties.Properties.BUG_REPORT_EMAIL_ADDRESSES.Trim() != string.Empty)
			{
				if (client.Account.Mail == string.Empty)
					client.Player.Out.SendMessage(LanguageMgr.GetTranslation(client.Account.Language, "Scripts.Players.Report.EmailHint"), eChatType.CT_Important, eChatLoc.CL_SystemWindow);
				else
				{
