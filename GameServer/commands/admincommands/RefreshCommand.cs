	...
	public class RefreshCommand : AbstractCommandHandler, ICommandHandler
	{
		...
		public void OnCommand(GameClient client, string[] args)
		{
			...
			else
			{
				...
				{
					DisplayMessage(client, T(client, "AdminCommands.Refresh.WrongModule"));
				}
				else
				{
					DisplayMessage(client, T(client, "AdminCommands.Refresh.Refreshing", method.Key));
					try
					{
						object value = method.Value.Invoke(null, new object[] { });
						if (value != null)
							DisplayMessage(client, T(client, "AdminCommands.Refresh.ReturnedValue", value));
					}
					catch(Exception e)
					{
						DisplayMessage(client, T(client, "AdminCommands.Refresh.Error", method.Key, e));
					}

					DisplayMessage(client, T(client, "AdminCommands.Refresh.Finished", method.Key));
				}
			}
		}
		...
		{
			DisplayMessage(client, T(client, "AdminCommands.Refresh.AvailableModules"));
