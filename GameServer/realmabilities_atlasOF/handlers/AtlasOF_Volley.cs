...
namespace DOL.GS.RealmAbilities
{
    public class AtlasOF_Volley : TimedRealmAbility
    {
        ...
        public override void Execute(GameLiving living)
        {
            ...
            {
                _player.Out.SendMessage(LanguageMgr.GetTranslation(_player.Client, "AtlasOF.Volley.CantUseInDungeons"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }

            if (_player.ActiveWeaponSlot != eActiveWeaponSlot.Distance || _player.ActiveWeapon == null)
            {
                _player.Out.SendMessage(LanguageMgr.GetTranslation(_player.Client, "AtlasOF.Volley.NeedBow"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }

            if (_player.rangeAttackComponent.UpdateAmmo(_player.ActiveWeapon) == null)
            {
                _player.Out.SendMessage(LanguageMgr.GetTranslation(_player.Client, "AtlasOF.Volley.NeedArrows"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }

            if (_player.CurrentRegion.GetZone(_player.GroundTarget.X, _player.GroundTarget.Y) == null)
            {
                _player.Out.SendMessage(LanguageMgr.GetTranslation(_player.Client, "AtlasOF.Volley.NeedGroundTarget"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }

            if (_player.IsWithinRadius(_player.GroundTarget, GetMinAttackRange(_player.Realm)))
            {
                _player.Out.SendMessage(LanguageMgr.GetTranslation(_player.Client, "AtlasOF.Volley.GroundTargetTooClose"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }

            if (!_player.IsWithinRadius(_player.GroundTarget, GetMaxAttackRange(_player.Realm)))
            {
                _player.Out.SendMessage(LanguageMgr.GetTranslation(_player.Client, "AtlasOF.Volley.GroundTargetTooFar"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            if (volley != null)
            {
                _player.Out.SendMessage(LanguageMgr.GetTranslation(_player.Client, "AtlasOF.Volley.AlreadyUsing"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
            ...
            {
                _player.Out.SendMessage(LanguageMgr.GetTranslation(_player.Client, "AtlasOF.Volley.CantUseWithCriticalShot"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
            else if (_player.rangeAttackComponent.RangedAttackType == eRangedAttackType.Long)
            {
                _player.Out.SendMessage(LanguageMgr.GetTranslation(_player.Client, "AtlasOF.Volley.CantUseWithLongshot"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                return;
            }
                ...
                foreach (AbstractArea area in _player.CurrentRegion.GetAreasOfSpot(_player.GroundTarget).Where(x => x is AbstractArea))
                {
                    if (area is Area.Circle)
                    {
                        if (_player.Realm == eRealm.Albion)
                        {
                            ...
                            {
                                _player.Out.SendMessage(LanguageMgr.GetTranslation(_player.Client, "AtlasOF.Volley.CantUseInEnemyBorderKeep"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                return;
                            }
                        }
                    ...
                    else if (area is KeepArea)
                    {
                        if (_player.Realm == eRealm.Albion)
                        {
                            ...
                            {
                                _player.Out.SendMessage(LanguageMgr.GetTranslation(_player.Client, "AtlasOF.Volley.CantUseInEnemyPortalKeep"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
                                return;
                            }
                        }
                        else if (_player.Realm == eRealm.Hibernia)
