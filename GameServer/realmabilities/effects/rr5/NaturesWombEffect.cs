	...
	public class NaturesWombEffect : TimedEffect
	{
		...
		private void OnAttack(DOLEvent e, object sender, EventArgs arguments)
		{
			...
			if (attackplayer != null)
				attackplayer.Out.SendMessage(LanguageMgr.GetTranslation(attackplayer.Client.Account.Language, "RealmAbility.NaturesWomb.AbsorbAttack", living.Name), eChatType.CT_SpellResisted, eChatLoc.CL_SystemWindow);
			int modheal = living.MaxHealth - living.Health;
			if (modheal > heal)
				modheal = heal;
			living.Health += modheal;
			if (player != null)
				player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "RealmAbility.NaturesWomb.ConvertAttack", modheal), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
