	...
	public class AcolyteTrainer : GameTrainer
	{
		...
		public override bool Interact(GamePlayer player)
		{
			...
			if (player.CharacterClass.ID == (int)TrainedClass)
			{
				...
				if (player.Level >= 5)
				{
					player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "AlbionTrainer.AcolytePaths", this.Name), eChatType.CT_Say, eChatLoc.CL_PopupWindow);
				}
				else
				{
		...
		public override bool WhisperReceive(GameLiving source, string text)
		{
			...
			switch (text)
			{
					...
					{
						this.SayTo(player, LanguageMgr.GetTranslation(player.Client.Account.Language, "AcolyteTrainer.ClassInfo.Cleric"));
					}
					else
					{
						this.SayTo(player, LanguageMgr.GetTranslation(player.Client.Account.Language, "StarterTrainer.ClassUnavailable", this.Name, "Cleric"));
					}
					return true;
					case "Friar":
					case "프라이어":
					...
					{
						this.SayTo(player, LanguageMgr.GetTranslation(player.Client.Account.Language, "AcolyteTrainer.ClassInfo.Friar"));
					}
					else
					{
						this.SayTo(player, LanguageMgr.GetTranslation(player.Client.Account.Language, "StarterTrainer.ClassUnavailable", this.Name, "Friar"));
					}
					return true;
					case "Heretic":
					case "헤러틱":
					...
					{
						this.SayTo(player, LanguageMgr.GetTranslation(player.Client.Account.Language, "StarterTrainer.ClassUnavailable", this.Name, "Heretic"));
					}
					return true;
