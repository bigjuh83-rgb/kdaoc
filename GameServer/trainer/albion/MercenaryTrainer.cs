	...
	public class MercenaryTrainer : GameTrainer
	{
		...
		public override bool Interact(GamePlayer player)
		{
			...
			else
			{
				...
				{
					player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "AlbionTrainer.MercenaryPrompt", this.Name), eChatType.CT_Say, eChatLoc.CL_PopupWindow);
		...
		public override bool WhisperReceive(GameLiving source, string text)
		{
			...
			if (CanPromotePlayer(player))
			{
				switch (text)
				{
					...
					case "그림자 길드 입단":
						player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "AlbionTrainer.MercenaryChooseWeapon", this.Name), eChatType.CT_Say,eChatLoc.CL_PopupWindow);
						break;
					case "slashing":
					case "베기":
