	...
	public class ForesterTrainer : GameTrainer
	{
		...
		public override bool Interact(GamePlayer player)
		{
			...
			if (player.CharacterClass.ID == (int) TrainedClass)
			{
				...
				if (player.Level>=5)
				{
						player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "HiberniaTrainer.ForesterPaths", this.Name), eChatType.CT_System, eChatLoc.CL_PopupWindow);
				}
				else
				{
		...
		public override bool WhisperReceive(GameLiving source, string text)
		{
			...
			switch (text) {
					...
					if(player.Race == (int) eRace.Celt || player.Race == (int) eRace.Firbolg || player.Race == (int) eRace.Sylvan){
						...
						player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "StarterTrainer.ClassUnavailable", this.Name, "Animist"), eChatType.CT_System, eChatLoc.CL_PopupWindow);
					}
					return true;
					case "Valewalker":
					case "발리워커":
					if(player.Race == (int) eRace.Celt || player.Race == (int) eRace.Firbolg || player.Race == (int) eRace.Sylvan){
						...
						player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "StarterTrainer.ClassUnavailable", this.Name, "Valewalker"), eChatType.CT_System, eChatLoc.CL_PopupWindow);
					}
					return true;
