	...
	public class StalkerTrainer : GameTrainer
	{
		...
		public override bool Interact(GamePlayer player)
		{
			...
			if (player.CharacterClass.ID == (int) TrainedClass)
			{
				...
				if (player.Level>=5)
				{
						player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "HiberniaTrainer.StalkerPaths", this.Name), eChatType.CT_System, eChatLoc.CL_PopupWindow);
				}
				else
				{
		...
		public override bool WhisperReceive(GameLiving source, string text)
		{
			...
			switch (text) {
					...
					if(player.Race == (int) eRace.Celt || player.Race == (int) eRace.Elf || player.Race == (int) eRace.Lurikeen || player.Race == (int) eRace.Shar){
						...
						player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "StarterTrainer.ClassUnavailable", this.Name, "Ranger"), eChatType.CT_System, eChatLoc.CL_PopupWindow);
					}
					return true;
					case "Nightshade":
					case "나이트셰이드":
					if(player.Race == (int) eRace.Elf || player.Race == (int) eRace.Lurikeen){
						...
						player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "StarterTrainer.ClassUnavailable", this.Name, "Nightshade"), eChatType.CT_System, eChatLoc.CL_PopupWindow);
					}
					return true;
