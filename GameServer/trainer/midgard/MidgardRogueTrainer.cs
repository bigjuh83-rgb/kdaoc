	...
	public class MidgardRogueTrainer : GameTrainer
	{
		...
		public override bool Interact(GamePlayer player)
		{
			...
			if (player.CharacterClass.ID == (int)TrainedClass)
			{
				...
				if (player.Level>=5)
				{
					player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "MidgardTrainer.RoguePaths", this.Name), eChatType.CT_Say, eChatLoc.CL_PopupWindow);
				}
				else
				{
		...
		public override bool WhisperReceive(GameLiving source, string text)
		{
			...
			switch (text) {
					...
					if(player.Race == (int) eRace.Dwarf || player.Race == (int) eRace.Kobold || player.Race == (int) eRace.Frostalf || player.Race == (int) eRace.Norseman || player.Race == (int) eRace.Valkyn){
						...
						player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "StarterTrainer.ClassUnavailable", this.Name, "Hunter"), eChatType.CT_Say, eChatLoc.CL_PopupWindow);
					}
					return true;
					case "Shadowblade":
					case "쉐도우블레이드":
					if(player.Race == (int) eRace.Kobold || player.Race == (int) eRace.Norseman || player.Race == (int) eRace.Valkyn){
						...
						player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "StarterTrainer.ClassUnavailable", this.Name, "Shadowblade"), eChatType.CT_Say, eChatLoc.CL_PopupWindow);
					}
					return true;
