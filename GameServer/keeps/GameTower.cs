...
namespace DOL.GS.Keeps
{
	...
	public class GameKeepTower : AbstractGameKeep
	{
		...
		public override bool CheckForClaim(GamePlayer player)
		{
			...
			{
				player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "Keep.MustBeInGroup"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
				return false;
			}

			if (player.Group.MemberCount < ServerProperties.Properties.CLAIM_NUM / 2)
			{
				player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "Keep.NeedPlayersToClaim", ServerProperties.Properties.CLAIM_NUM / 2), eChatType.CT_System, eChatLoc.CL_SystemWindow);
				return false;
			}
