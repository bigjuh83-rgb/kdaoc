...
namespace DOL.GS.ServerRules
{
	...
	if(previousInstance.Skin != targetPoint.TargetRegion)
	{
		...
		{
			//We can't jump !
			player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "ServerRules.Instance.AlreadyRunning", previousInstance.Description), eChatType.CT_System, eChatLoc.CL_SystemWindow);
			return false;
		}
		else
		{
	...
	player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client, "AdventureWingJumpPoint.InstanceCreateError"), eChatType.CT_System, eChatLoc.CL_SystemWindow);
            return false;
        }
	}
}
