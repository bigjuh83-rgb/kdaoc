...
namespace DOL.GS.Spells
{
	...
	public class RangeSpellHandler : PrimerSpellHandler
	{
		public override bool CheckBeginCast(GameLiving selectedTarget)
		{
			...
			if(UninterruptableSpell != null) { MessageToCaster(LanguageMgr.GetTranslation((Caster as GamePlayer)?.Client.Account.Language, "Warlock.Range.AlreadyPreparingUninterruptible"), eChatType.CT_System); return false; }
            GameSpellEffect PowerlessSpell = SpellHandler.FindEffectOnTarget(Caster, "Powerless");
			if(PowerlessSpell != null) { MessageToCaster(LanguageMgr.GetTranslation((Caster as GamePlayer)?.Client.Account.Language, "Warlock.Range.AlreadyPreparingPowerless"), eChatType.CT_System); return false; }
            GameSpellEffect RangeSpell = SpellHandler.FindEffectOnTarget(Caster, "Range");
            if (RangeSpell != null) { MessageToCaster(LanguageMgr.GetTranslation((Caster as GamePlayer)?.Client.Account.Language, "Warlock.Range.MustFinishRange"), eChatType.CT_System); return false; }
            return true;
		}
		/// <summary>
